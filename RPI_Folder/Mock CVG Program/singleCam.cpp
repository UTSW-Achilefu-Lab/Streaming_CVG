...
int main()
{
    ...
    /*
        Pipeline that works
        gst-launch-1.0 libcamerasrc camera-name="/base/soc/i2c0mux/i2c@1/imx219@10" ! video/x-raw,width=640,height=480,framerate=30/1 ! videoconvert ! video/x-raw ! queue ! autovideosink
    */
    ...
    cv::VideoCapture capIR(irCameraPipeline, cv::CAP_GSTREAMER);
    ...
    while (true)
    {
        ...
        yenThresholdedFrame = ImgProc_YenThreshold(irFrames, false, foundThresh);
        ...
        cv::Mat irFramesBGRA;
        cv::cvtColor(irFrames, irFramesBGRA, cv::COLOR_BGR2BGRA);
        cv::hconcat(*ColoredFrame, irFramesBGRA, combineIRFrames);
        cv::imshow("Frames", combineIRFrames);
